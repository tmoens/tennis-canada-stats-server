...
export class TournamentGradeApprovalController {
  constructor(private readonly service: TournamentGradeApprovalService) {}
  ...
  async create(
    ...
    @Param('tournamentCode') tournamentCode: string,
    @Param('approvedGrade') approvedGrade: string,
  ): Promise<TournamentGradeApproval | null> {
    return this.service.create(tournamentCode, approvedGrade, req.user);
  }
}
