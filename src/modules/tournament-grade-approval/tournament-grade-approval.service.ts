...
export class TournamentGradeApprovalService {
  constructor(
    @InjectRepository(TournamentGradeApproval)
    private readonly repo: Repository<TournamentGradeApproval>,
  ) {}

  async getMostRecentApproval(
    tournament: string,
  ): Promise<TournamentGradeApproval | null> {
    const mostRecentApproval: TournamentGradeApproval = await this.repo
  ...
  async create(tournament: string, level: string, approvingUser: User) {
    const approval = new TournamentGradeApproval();
    approval.tournament = tournament;
    approval.approvedLevel = level;
    approval.approvingUser = approvingUser.name;
