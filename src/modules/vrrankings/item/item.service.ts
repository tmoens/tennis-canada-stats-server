...
export class VRRankingsItemService {
  ...
  async findByPubAndPlayer(
    player: Player,
    publication: VRRankingsPublication,
  ): Promise<VRRankingsItem> {
  ...
  async importVRRankingsListFromVR(
    ...
    let list = await this.vrapi.get(
      'Ranking/' +
        publication.rankingsCategory.typeCode +
        '/Publication/' +
        publication.publicationCode +
        '/Category/' +
        publication.rankingsCategory.categoryCode,
    );
    ...
    for (const apiListItem of list) {
      ...
      try {
        ...
        logger.error(
          '281312450 Error saving rankings item: ' +
  ...
  async renumberPlayer(fromPlayer: Player, toPlayer: Player): Promise<number> {
    const items: VRRankingsItem[] = await this.repository.find({
      where: {
    ...
    logger.info(
      'Renumbered player in Rankings Data (' +
        items.length +
        ' times) from ' +
        fromPlayer.playerId +
        ' to ' +
