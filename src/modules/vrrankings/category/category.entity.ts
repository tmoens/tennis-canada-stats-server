...
export class VRRankingsCategory {
  @Column('varchar', {
    ...
    primary: true,
    comment:
      'VR UUID for the ranking category (M 4.5 singles/W O65 doubles/Girls U16 singles, etc.).',
  })
  categoryCode: string;

  @ManyToOne(
    () => VRRankingsType,
    (vrRankingsType) => vrRankingsType.vrRankingsCategories,
    {
      nullable: false,
      onDelete: 'CASCADE',
      onUpdate: 'CASCADE',
  ...
  @OneToMany(
    ...
    (publications) => publications.rankingsCategory,
    { onDelete: 'CASCADE' },
  )
