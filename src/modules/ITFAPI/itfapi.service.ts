...
export class ItfapiService {
  ...
  async get(pattern: string = ''): Promise<any[]> {
    ...
    try {
      ...
      logger.error(
        'Error calling IT API: ' +
          this.config.itfapiURL +
          pattern +
          ' Error: ' +
          e,
      );
  ...
  async loadResults() {
    ...
    const startLoadingFrom: Moment = moment().subtract(
      this.config.itfLoaderPeriod,
      'days',
    );
    ...
    for (const discipline of disciplines) {
      ...
      for (const playerData of results) {
        ...
        const ep: ExternalPlayer =
          await this.externalPlayerService.loadFromITFAPI(playerData.Biography);
        ...
        for (const disciplineData of playerData.Disciplines) {
          ...
          for (const resultData of disciplineData.Results) {
            ...
            const et: ExternalTournament =
              await this.externalTournamentService.loadFromITFAPI(resultData);
            if (et) {
            ...
            for (const eventData of resultData.Events) {
              ...
              const ee: ExternalEvent =
                await this.externalEventService.loadFromITFAPI(et, eventData);
              if (ee) {
              ...
              const eer: ExternalEventResult =
                await this.externalEventResultService.loadFromITFAPI(
                  ee,
                  ep,
                  eventData,
                );
              if (eer) {
                ...
                logger.warn(
                  'Unable to load event result for player: ' +
                    ep.playerId +
                    '. Event: ' +
                    eventData.EventId +
                    '/' +
              ...
              for (const matchData of eventData.Matches) {
                await this.itfMatchResultService.loadFromITFAPI(
                  ee,
                  ep,
                  matchData,
                );
