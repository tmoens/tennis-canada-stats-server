...
export const TOURNAMENT_URL_PREFIX =
  'https://tc.tournamentsoftware.com/sport/tournament.aspx?id=';
...
export class MatchDataExporterService {
  ...
  async buildUTRReport(): Promise<JobStats> {
    ...
    logger.info(
      'Querying UTR Data for tournaments updated since ' + updatedSinceString,
    );
    ...
    for (const t of tournaments) {
      ...
      logger.info(
        `UTR reporting: tournament ${t.tournamentCode} ${t.startDate}`,
      );
      for (const e of t.events) {
        ...
        if (
          t.isTournament() &&
          'Junior' === e.vrRankingsCategory.vrRankingsType.typeName &&
          e.maxAge < 12
        ) {
  ...
  async buildMatchCompetitivenessReport() {
    ...
    for (const tournament of tournaments) {
      ...
      if (reportData.length > 0) {
        ...
        tournamentsWithoutMatches.push({
    ...
    utils.book_append_sheet(wb, reportSheet, 'Matches');
    const tournamentsWithoutMatchesSheet = utils.json_to_sheet(
      tournamentsWithoutMatches,
    );
    utils.book_append_sheet(
      wb,
      tournamentsWithoutMatchesSheet,
      'TournamentsWithoutMatches',
    );
    const now = moment().format('YYYY-MM-DD-HH-mm-ss');
