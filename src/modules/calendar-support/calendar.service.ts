...
export class CalendarService {
  ...
  constructor(
    ...
    @InjectDataSource('calendar') private readonly dataSource: DataSource,
  ...
  async sync(): Promise<boolean> {
    this.tournamentRepo = this.dataSource.getRepository(CalendarTournament);
    this.eventRepo = this.dataSource.getRepository(CalendarEvent);
    this.jobStats = new JobStats('calendarDbSync');
    ...
    let d = new Date();
    d = new Date(
      d.setDate(d.getDate() - this.configService.calendarDbSyncPeriod),
    );
    const updatedSinceString = d.toISOString().slice(0, 10);
    const tournaments: Tournament[] =
      await this.tournamentService.getTournamentsUpdatedSince(
        updatedSinceString,
      );
    ...
    for (const t of tournaments) {
      ...
      const calendarT: CalendarTournament = await this.tournamentRepo
      ...
      if (!calendarT) {
        const tournament: Tournament =
          await this.tournamentService.getTournamentWithEventsAndLicense(
            t.tournamentCode,
          );
        ...
        if (
          calendarT.lastUpdated.toISOString() !==
          t.lastUpdatedInVR.toISOString()
        ) {
      ...
      if (this.jobStats.get('done') % 100 === 0) {
        logger.info(
          `${this.jobStats.get('done')} tournaments synced to the calendar database.`,
        );
      }
    }
  ...
  async createCalendarTournamentFromTournament(
    t: Tournament,
