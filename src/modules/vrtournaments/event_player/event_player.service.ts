...
export class EventPlayerService {
  ...
  async loadEntry(event: Event, entryData: any, jobStats: JobStats) {
    if (entryData && entryData.MemberID) {
      const player: Player = await this.playerService.findPlayerOrFacsimile(
        { playerId: entryData.MemberID },
        false,
      );
      if (player) {
        jobStats.bump('EventEntryCreated');
        const entry: EventPlayer = new EventPlayer(event, player);
        this.repository.save(entry).catch(() => {
          logger.error(
            'Failed to save an EventPlayer: ' +
  ...
  async renumberPlayer(fromPlayer: Player, toPlayer: Player): Promise<number> {
    const entries: EventPlayer[] = await this.repository.find({
      where: {
    ...
    for (entry of entries) {
      ...
      await this.repository.save(entry).catch((reason) => {
        logger.error(
          'Failed to renumber eventPlayer record. ' +
            'Reason: ' +
            reason +
            'eventPlayer: ' +
    ...
    logger.info(
      'Renumbered player in Event Data (' +
        entries.length +
        ' times) from ' +
        fromPlayer.playerId +
        ' to ' +
