...
export class Event {
  ...
  @OneToMany(() => Draw, (draws) => draws.event, { onDelete: 'CASCADE' })
  draws: Draw[];

  @OneToMany(() => Match, (matches) => matches.event, { onDelete: 'CASCADE' })
  matches: Match[];

  @OneToMany(() => EventPlayer, (eventPlayers) => eventPlayers.event, {
    onDelete: 'CASCADE',
    onUpdate: 'CASCADE',
  })
  players: EventPlayer[];
  ...
  buildFromVRAPIObj(apiEvent: any) {
    ...
    this.maxAge =
      null != apiEvent.MaxAge
        ? (this.maxAge = parseInt(apiEvent.MaxAge, 10))
        : 0;
    this.minAge =
      null != apiEvent.MinAge
        ? (this.minAge = parseInt(apiEvent.MinAge, 10))
        : 0;
    this.winnerPoints =
      null != apiEvent.Points1
        ? (this.winnerPoints = parseInt(apiEvent.Points1, 10))
        : 0;
    this.isSingles = 1 === parseInt(apiEvent.GameTypeID, 10);
    if (null != apiEvent.Grading) this.grade = apiEvent.Grading.Name;
  }

  // The API does not give the VR rankings category of the event.  So,
  // we use what we know about the event to build a simple rankings
  // category name (which we call a categoryId). The categoryId can then
  // be used to look up the VR rankings category code.
  // This works quite well for most events in most tournaments, but in
  // things like box ladders where and event can be something like
  // Mixed Rookie Under 15 Singles for which there is no rankings category
  // the subsequent lookup of a VR Rankings Category will fail, of course,
  // because no such rankings category exists.
