...
export class MatchPlayerService {
  ...
  async importMatchPlayersFromVR(
    match: Match,
    matchData: any,
  ): Promise<boolean> {
    let player: Player;
  ...
  failedToSaveMatchPlayer(mp: MatchPlayer, reason: any) {
    logger.error(
      'Failed to save matchPlayer record. ' +
        'Reason: ' +
        reason +
        'matchPlayer: ' +
  ...
  // Support the renumbering of a player for whom match data already exists.
  ...
  async renumberPlayer(fromPlayer: Player, toPlayer: Player): Promise<number> {
    const matchPlayers: MatchPlayer[] = await this.repository.find({
      where: {
    ...
    for (mp of matchPlayers) {
      ...
      await this.repository.save(mp).catch((reason) => {
        logger.error(
          'Failed to renumber matchPlayer record. ' +
            'Reason: ' +
            reason +
            'matchPlayer: ' +
            JSON.stringify(mp, null, 2),
    ...
    logger.info(
      'Renumbered player in Match Data (' +
        matchPlayers.length +
        ' times) from ' +
        fromPlayer.playerId +
        ' to ' +
